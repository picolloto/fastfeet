...
class OrderController {
    ...
    async store(req, res) {
        ...
        const deliverymanOrder = await Deliveryman.findByPk(deliveryman_id);

        await deliverymanOrder.save();

        const retorno = await Queue.add(OrderMail.key, {
            deliverymanOrder,
        });
    ...
    async update(req, res) {
        ...
        const orderVerify = await Order.findOne({
        ...
        const deliverymanDelivery = await Order.findAll({
        ...
        const countDelivery = deliverymanDelivery.map((row) => {
            if (
                row.start_date &&
                isWithinInterval(row.start_date, {
                    start: startHour,
                    end: endHour,
                })
            ) {
        ...
        if (req.body.start_date && countDelivery < 5) {
            if (
                !isWithinInterval(hourToday, { start: startHour, end: endHour })
            ) {
            ...
            return res.status(500).json({
                error:
                    'Você já atingiu o limite de entregas hoje, volte amanhã!',
            });
        }
    ...
    async delete(req, res) {
        ...
        if (description === null) {
            description =
                'Sentimos muito, mas a entrega teve que ser cancelada!';
        }
        ...
        await Queue.add(OrderDeliveryMail.key, {
            name,
            email,
            product,
            description,
